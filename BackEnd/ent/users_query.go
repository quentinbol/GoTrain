...
func (uq *UsersQuery) QueryMessages() *MessagesQuery {
...
func (uq *UsersQuery) WithMessages(opts ...func(*MessagesQuery)) *UsersQuery {
	query := (&MessagesClient{config: uq.config}).Query()
...
func (uq *UsersQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Users, error) {
	...
	if query := uq.withMessages; query != nil {
		if err := uq.loadMessages(ctx, query, nodes, nil,
			func(n *Users, e *Messages) { n.Edges.Messages = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UsersQuery) loadMessages(ctx context.Context, query *MessagesQuery, nodes []*Users, init func(*Users), assign func(*Users, *Messages)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Users)
	for i := range nodes {
		if nodes[i].messages_user == nil {
			continue
		}
		fk := *nodes[i].messages_user
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "messages_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
